	...
	for equation in symbol_payload["equations"]:
		...
		for symbol in equation["symbols"]:
			query, results = retrieve_symbol_evidence(
				service, paper_id, equation["equation_id"], symbol, top_k
			)
			record = extract_symbol_meaning(
				symbol, equation["equation_id"], results,
				equation=equation["latex"], nlp=nlp, classifier=classifier,
			).to_dict()
...
def extract_all_symbol_meanings(
	...
	paper_count = symbol_count = definition_count = 0
	for symbol_file in sorted(symbols_dir.glob("*.json")):
		payload = json.loads(symbol_file.read_text(encoding="utf-8"))
		if payload["paper_id"] != symbol_file.stem:
			raise ValueError(f"Invalid symbol paper: {symbol_file.name}")
		output = extract_paper_symbol_meanings(
			service, payload, top_k, nlp=nlp, classifier=classifier
		)
		_write_json(output, output_dir / symbol_file.name)
		paper_count += 1
